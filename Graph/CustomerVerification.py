...
class CustomerVerification:
    def __init__(self, model_name: str = "openai/gpt-oss-120b"):
        ...
        self.verificationModel = ChatGroq(model=model_name).with_structured_output(CustomerData)
    ...
    def verify_customer(self, state : GraphInnerState) -> GraphInnerState:
        ...
        while not customerDetails:
            ...
            elif not structured_response.customer_id and not structured_response.customer_phone_number and not structured_response.customer_email:
                interrupt_result = interrupt("Please provide your phone number or email for verification.")
            else:
                interrupt_result = interrupt("The provided information does not match our records. Please try again.")
            structured_response = self.verificationModel.invoke(
                [
                    SystemMessage(content=system_prompt),
